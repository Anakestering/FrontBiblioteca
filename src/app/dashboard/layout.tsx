    ...
    <div className="min-h-screen flex bg-[var(--surface)]">
      ...
      <aside className={`fixed inset-y-0 left-0 z-50 w-64 bg-white dark:bg-[#161b22] border-r border-[var(--border)] flex flex-col transform transition-transform duration-300 lg:translate-x-0 lg:static lg:flex ${sidebarOpen ? 'translate-x-0' : '-translate-x-full'}`}>
        ...
        <nav className="flex-1 px-3 py-4 space-y-1 overflow-y-auto">
          {isAdmin ? (
            // Admin: botão Nova Reserva + seção "Administração"
            <>
              <Link
                href="/dashboard/usuario/reservar"
                onClick={() => setSidebarOpen(false)}
            ...
            // Usuário comum: exibe apenas itens não-admin (sem label de seção)
        ...
        <div className="px-3 py-4 border-t border-[var(--border)]">
          <Link
            ...
            onClick={() => setSidebarOpen(false)}
            className="flex items-center gap-3 px-3 py-2.5 rounded-lg bg-[var(--surface-2)] hover:bg-[var(--border)] transition-colors group"
          >
            ...
            <div className="flex-1 min-w-0">
              ...
              <p className="text-[10px] text-[var(--text-muted)]">{user.tipo === 'ADMIN' ? 'Administrador' : 'Usuário'}</p>
            </div>
            <svg className="w-3.5 h-3.5 text-[var(--text-muted)] opacity-0 group-hover:opacity-100 transition-opacity shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor">
              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 5l7 7-7 7" />
            </svg>
          </Link>
          <div className="flex items-center justify-between mt-2 px-1">
            <ThemeToggle />
            <button onClick={logout} className="flex items-center gap-1.5 text-xs text-[var(--text-muted)] hover:text-rose-500 transition-colors px-2 py-1.5 rounded-md hover:bg-rose-50 dark:hover:bg-rose-900/20">
      ...
      <div className="flex-1 flex flex-col min-w-0">
        ...
        <main className="flex-1 p-4 lg:p-8 overflow-auto fade-in">
          {children}
        </main>
      </div>
    </div>
