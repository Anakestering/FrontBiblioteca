...
// Verifica se todos os blocos selecionados formam um único grupo consecutivo
...
function BuscaUsuario({ usuarios, onSelecionar }: {
  usuarios: Usuario[];
  onSelecionar: (u: Usuario) => void;
}) {
  ...
  const filtrados = busca.length >= 1
    ? usuarios.filter(u =>
      ...
      u.cpf.replace(/\D/g, '').includes(busca.replace(/\D/g, '')) ||
    ...
    <div className="space-y-2">
      ...
      {busca.length >= 1 && (
        <div className="border border-[var(--border)] rounded-xl overflow-hidden">
          {filtrados.length === 0 ? (
            <p className="text-sm text-[var(--text-muted)] px-4 py-3 text-center">
            ...
            filtrados.map(u => (
              <div key={u.id}
                ...
                <div className="min-w-0">
                  ...
                  <p className="text-xs text-[var(--text-muted)]">CPF: {u.cpf} · {u.email}</p>
                </div>
                <button
                  type="button"
...
export default function ReservarPage() {
  ...
  useEffect(() => {
    ...
    if (isAdmin) {
      fetches.push(
        usuariosApi.listar().then(lista =>
          setTodosUsuarios(lista.filter(u => u.ativo && u.nivelAcesso !== 'ADMIN'))
        )
      );
    }
  ...
  //
  // A API retorna apenas o horário de INÍCIO de cada reserva.
  // Para exibir todos os blocos ocupados corretamente, expandimos cada início
  // até o fim da reserva em intervalos de DURACAO minutos.
  ...
  const buscarOcupados = useCallback(async () => {
    ...
    try {
      const resultados = await Promise.all(
        itensSelecionados.map(async (id) => {
          ...
          dados.forEach(entrada => {
            ...
            if (fimISO) {
              // Expandir todos os blocos entre início e fim
              ...
              // Sem fim: marca apenas o bloco de início
      ...
      // Remove blocos selecionados que ficaram ocupados
  ...
  const maxPessoas = (() => {
    ...
    if (tipo === 'computador') {
      const selecionados = pcs.filter(p => itensSelecionados.includes(p.id));
      return selecionados.reduce((acc, p) => acc + p.capacidadePessoas, 0);
    } else {
      const selecionadas = sls.filter(s => itensSelecionados.includes(s.id));
      return selecionadas.reduce((acc, s) => acc + s.capacidadePessoas, 0);
    }
  })();
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (isAdmin && !usuarioVinculado) {
      setError('Vincule um usuário à reserva.');
      return;
    }
    ...
    if (!saoConsecutivos(blocosSelecionados)) {
      setError(
        'Os horários selecionados precisam ser consecutivos. Selecione blocos em sequência, sem intervalos entre eles.'
      );
      return;
    }
    ...
    try {
      ...
      setSuccess(
        ...
        (precisaAprovacao ? ' Aguardando aprovação da secretaria.' : '')
      );
        ...
        <div className="p-4 rounded-xl bg-amber-50 dark:bg-amber-900/20 border border-amber-200 dark:border-amber-800 flex items-start gap-3">
          ...
          <div>
            ...
            <p className="text-xs text-amber-700 dark:text-amber-400 mt-0.5">
              ...
              Esta reserva ficará pendente até aprovação da secretaria.
            </p>
          </div>
        </div>
      )}
              ...
              <div className="flex items-center justify-between p-3 rounded-xl bg-blue-50 dark:bg-blue-900/20 border border-blue-200 dark:border-blue-700">
                <div>
                  ...
                  <p className="text-xs text-blue-600 dark:text-blue-400 mt-0.5">
                    CPF: {usuarioVinculado.cpf} · {usuarioVinculado.email}
                  </p>
                </div>
                <button
                  type="button"
                  onClick={() => setUsuarioVinculado(null)}
        ...
        {itensSelecionados.length > 0 && (
          <Secao numero={3} titulo="Quantidade de pessoas">
            ...
            <p className="text-xs text-[var(--text-muted)]">
              Máximo {maxPessoas} pessoa{maxPessoas !== 1 ? 's' : ''}
              {itensSelecionados.length > 1 && ` (soma da capacidade dos ${tipo === 'computador' ? 'computadores' : 'salas'} selecionados)`}
            </p>
          </Secao>
        )}
              ...
              {diasDisponiveis.map(dia => {
                ...
                return (
                  <button key={key} type="button"
                    ...
                    className={`shrink-0 px-3 py-2 rounded-xl border-2 text-center transition-all min-w-[60px] ${sel
                      ? 'border-blue-600 bg-blue-50 dark:bg-blue-900/20'
                      : 'border-[var(--border)] hover:border-blue-300'}`}>
                    ...
                    <p className={`text-sm font-bold ${sel ? 'text-blue-700 dark:text-blue-400' : 'text-[var(--text-primary)]'}`}>
                      {dia.getDate()}
                    </p>
          ...
          <Secao numero={5} titulo="Escolha os horários">
            {loadingOcupados ? (
              ...
              <>
                ...
                {/* Aviso de horários não consecutivos */}
                ...
                {/* Resumo dos blocos selecionados */}
                {grupos.length > 0 && (
                  <div className="mt-2 pt-3 border-t border-[var(--border)] space-y-2">
                    ...
                    {itensSelecionados.length > 0 && (
                      <p className="text-xs text-blue-600 font-medium">
                        ...
                        {precisaAprovacao ? ' — aguardam aprovação da secretaria' : ''}
                      </p>
                    )}
                  </div>
                )}
